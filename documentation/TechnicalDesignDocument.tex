...
\begin{document}
    ...
    \subsubsection{Marama Joints}
    When working with multiple Marama objects, one of the most important things to do is keeping track of the places where they can be attached to other objects.
    This information is required to fit multiple objects against one another, to know where to draw a newly created Marama, to be able to know which joint a user meant to interact with, etc.


    The replacement thought up for this method is the following:
    Every object gets its own array which contains the middle points of the various joint, listed with a vector3 from the object's centre.
    These vectors could be rotated along with the object if need be.
    If you take the above step of comparing the collision point of your mouse with the centre of your object, you would afterwards be able to check the dot product of your collisionpoint against the origins of the faces.
    ...
    For this you can use another vector3, to determine the direction the joint should face.
    ...
    Copying all this over when using polymorphic objects seemed like it would get slow and messy, however.
    Instead, the program looks at marama types.
    ...
    Another Marama could re-use that circle joint, or the triangle joint within, without having to copy everything over.
